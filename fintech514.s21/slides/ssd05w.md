...
- `sudo apt-get update` to get synchronized with the official repositories

- `sudo apt-get upgrade` to bring everything up to date

- `sudo apt-get install screenfetch` installs the `screenfetch` program

- `sudo apt-get install -y screenfetch` won't ask you for confirmation

- Useless, but fun: `sudo apt-get install -y sl`
...
# Gettings Things From the Interwebs

- `curl` is good here, with `--output` option to save the retrieved file
...
- `sudo apt-get install git` if you don't have `git` already
